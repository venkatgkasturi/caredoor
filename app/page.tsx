...
import { BellRing, CalendarDays, Check, ChevronRight, Clock3, DoorOpen, Eye, HeartHandshake, Home, LockKeyhole, MessageCircle, Phone, Play, Radio, Settings, ShieldCheck, Sparkles, UsersRound, Video, Wifi } from "lucide-react";
...
export default function HomePage() {
  ...
  const statusCopy = useMemo(() => {
    ...
    return "No matching visit found";
  }, [resolution]);
  ...
  useEffect(() => {
    ...
    const register = async () => {
      await context.registerTool({
        ...
        description: "Run the CareDoor demo flow for a new unmatched front-door event.",
        inputSchema: { type: "object", properties: {}, additionalProperties: false },
        annotations: { readOnlyHint: false, untrustedContentHint: false },
        execute: async () => { simulateEvent(); return { status: "received", classification: "unmatched_visitor" }; },
      ...
      <main className="min-h-screen lg:pl-[252px]">
        <header className="sticky top-0 z-20 flex h-[76px] items-center justify-between border-b border-[var(--line)] bg-[color:var(--canvas)/0.92] px-5 backdrop-blur-xl sm:px-8 lg:px-10">
          ...
          <div className="flex items-center gap-2.5">
            <Button onClick={simulateEvent} className="h-10 rounded-xl bg-[var(--navy)] px-4 text-white hover:bg-[#1d3146]"><Play className="size-4 fill-current" /><span className="hidden sm:inline">Simulate Ring event</span><span className="sm:hidden">Simulate</span></Button>
        ...
        <div className="mx-auto max-w-[1480px] px-5 py-7 sm:px-8 lg:px-10 lg:py-9">
          ...
          <div className="grid items-start gap-7 xl:grid-cols-[minmax(0,1.45fr)_minmax(340px,.75fr)]">
            <div className="space-y-7">
              <section>
                <div className="mb-3 flex items-center justify-between"><div><p className="eyebrow">Needs attention</p><h2 className="mt-1 text-xl font-bold tracking-[-0.03em] text-[var(--navy)]">One doorstep event</h2></div><button className="text-sm font-semibold text-[var(--navy)] hover:underline" type="button">View all</button></div>
                {eventVisible ? (
